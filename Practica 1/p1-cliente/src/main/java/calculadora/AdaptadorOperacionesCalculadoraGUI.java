...
public class AdaptadorOperacionesCalculadoraGUI implements ICalculadora {
	...
	public AdaptadorOperacionesCalculadoraGUI() {
		try {
			TTransport transport = new TSocket("localhost",PUERTO);
			transport.open();
	...
	*/
	...
	public double obtenerUltimoResultado() {
		try {
			return cliente.ultimoResultado();
		} catch (TException e) {
			// TODO Auto-generated catch block
			return Double.NaN;
		}
	...
	public double multiplicar(double operando1, double operando2) {
		try {
			return cliente.multiplicar(operando1, operando2);
		} catch (TException e) {
			// TODO Auto-generated catch block
			return Double.NaN;
		}
	...
	public double dividir(double dividendo, double divisor) throws Exception {
		try {
			return cliente.dividir(dividendo, divisor);
		}catch (TException e) {
			return Double.NaN;
		}
	}
